...
VulkanImage::VulkanImage(std::shared_ptr<VulkanDevice> device, const Description& description)
    ...
    const auto memory_type_index = m_device->physical_device().find_memory_type(
    ...
    VK_CHECK(vkBindImageMemory(m_device->handle(), m_image, m_memory, 0))
}
